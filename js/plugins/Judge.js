...
(function(){
    ...
    Game_Map.prototype.init = function(){
        ...
        var queen = null;
        this._events.forEach(function(element){
            if(element.eventType !== undefined){
                switch(element.eventType){
                    case 'queen':
                        queen = element;
                        $gamePlayer.judge.attachBeatEventListener(queenMoveUpward.bind(element));
                        break;
                    case 'zombie':
                        element.listener = $gamePlayer.judge.attachBeatEventListener(zombieMoveTowardsQueen.bind(element, queen));
                        break;
                }
            }
        });
...
//Move Route functions
var queenMoveUpward = function(){
    $gamePlayer.judge.nextBeatIndex % 4 == 0 && this.moveStraight(8);
}

var zombieMoveTowardsQueen = function(queenEvent){
    if($gamePlayer.judge.nextBeatIndex % 2 == 0){
        var deltaX = queenEvent._x - this._x;
        var deltaY = queenEvent._y - this._y;

        if(Math.abs(deltaX) > 0){
            this.moveStraight(4 + 2 * Number(deltaX > 0));
        }else if(Math.abs(deltaY) > 0){
            this.moveStraight(2 + 6 * Number(deltaY < 0));
        }
        
        if(Math.abs(deltaX) + Math.abs(deltaY) <= 1){
            queenEvent._interpreter._params[0] = queenEvent.eventId();
            queenEvent._interpreter._params[1] = 1;
            queenEvent._interpreter.command212();
            $gameVariables.setValue(1, $gameVariables.value(1) - 1);
        }
    }
}

var flameUpdate = function(isOn, SEpitch){
    if(isOn){
        var key = [$gameMap._mapId, this._eventId, "A"];
        $gameSelfSwitches.setValue(key,false);
        key = [$gameMap._mapId, this._eventId, "B"];
        $gameSelfSwitches.setValue(key,true);
        AudioManager.playSe({name: "Fire1", pan:0, pitch: SEpitch, volume: 50});
    }else{
        var key = [$gameMap._mapId, this._eventId, "A"];
        $gameSelfSwitches.setValue(key,true);
        key = [$gameMap._mapId, this._eventId, "B"];
...
var castUpdate = function(isOn, SEpitch, SEvolume){
    if(isOn){
        var key = [$gameMap._mapId, this._eventId, "A"];
        $gameSelfSwitches.setValue(key,false);
        key = [$gameMap._mapId, this._eventId, "B"];
        $gameSelfSwitches.setValue(key,true);
        AudioManager.playSe({name: "Hammer", pan:0, pitch: SEpitch, volume: SEvolume});
    }else{
        var key = [$gameMap._mapId, this._eventId, "A"];
        $gameSelfSwitches.setValue(key,true);
        key = [$gameMap._mapId, this._eventId, "B"];
